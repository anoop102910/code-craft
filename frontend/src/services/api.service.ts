...
import { UserResponse, ProblemResponse, ProblemRequest, LoginResponse, ProblemsResponse } from '@/types/index';
...
export const apiService = {
  auth: {
    ...
    register: async (data: { username: string; email: string; password: string }): Promise<LoginResponse> => 
      (await api.post('/auth/register', data)).data,
    getProfile: async (): Promise<UserResponse> => 
      (await api.get('/auth/me')).data,
  },
  problems: {
    getAll: async (): Promise<ProblemsResponse> => 
      (await api.get('/problems')).data,
